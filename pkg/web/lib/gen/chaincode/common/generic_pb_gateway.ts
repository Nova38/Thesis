...
import type { IMessageTypeRegistry, JsonWriteStringOptions, PartialMessage } from '@bufbuild/protobuf'
...
export class GenericServiceClient {
   ...
   */
  async listByCollection(req: PartialMessage<ListByCollectionRequest>): Promise<ListByCollectionResponse> {
    const msg = req instanceof ListByCollectionRequest ? req : new ListByCollectionRequest(req)
    const results = utf8Decoder.decode(
      await this.contract.evaluateTransaction(
        'ListByCollection',
        msg.toJsonString(this.jsonWriteOptions),
