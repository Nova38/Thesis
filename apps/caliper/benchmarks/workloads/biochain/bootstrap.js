...
class BootstrapWorkload extends WorkloadModuleBase {
  ...
  async initializeWorkloadModule(
    ...
    helper.logger.info('Args', args)
    helper.logger.info('CollectionId', this.collectionId)

    const { bootstrap, create } = saacs.BootstrapBiochainRequests(
      this.collectionId,
    )
    ...
    this.requests = this.requests = [
      ...create.map((x) => {
        return {
          key: x.item.key,
          req: this.ArgsBuilder.items.create(x, this.requestOptions),
        }
      }),
    ]
  }
  ...
  async submitTransaction() {
    helper.logger.info(this.requests)

    const txStatus = await this.sutAdapter.sendRequests(this.bootstrap)

    helper.logger.info('txStatus', txStatus)

    const txCreateStatus = []
    for (const req of this.requests) {
      helper.logger.info('submitting', req.key)
      const status = await this.sutAdapter.sendRequests(req.req)
      helper.logger.info('item', req.key, 'status', status)

      txCreateStatus.push(status)
    }

    // return txStatus
    helper.logger.info('txCreateStatus', txCreateStatus)

    return { txStatus, txCreateStatus }
  }
}
