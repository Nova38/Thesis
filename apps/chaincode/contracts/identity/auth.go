...
import (
	...
	policy2 "github.com/nova38/saacs/apps/chaincode/policy"
	"github.com/nova38/saacs/apps/chaincode/state"
...
func (ctx *TxCtx) authorized(op *authpb.Operation) (bool, error) {
	...
	if valid, err := policy2.ValidateOperation(col, op); err != nil {
	...
	if auth, err := policy2.AuthorizedPolicy(col.GetDefault(), op); err != nil {
		return false, oops.Wrap(err)
	} else if auth {
	...
	if auth, err := policy2.AuthorizedPolicy(membership.GetPolices(), op); err != nil {
		return false, oops.Wrap(err)
	} else if auth {
