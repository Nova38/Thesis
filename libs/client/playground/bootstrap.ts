import { Any, FieldMask, Message, Timestamp, createRegistry } from '@bufbuild/protobuf'
...
async function MakeCollection() {
  ...
  const collectionRequest = new pb.BootstrapRequest({
    collection: new pb.Collection({
      ...
      default: {
        ...
        itemPolicies: {
          ...
          [pb.UserCollectionRoles.typeName]: {
            actions: [pb.Action.VIEW, Action.SUGGEST_VIEW],
            allowSubPaths: false,
            fullPath: '',
            path: '',
          },
  ...
  const response = await service.get(
    new pb.GetRequest({
      key: new pb.ItemKey({
        collectionId,
        itemType: pb.Collection.typeName,
...
async function GetUserRoles() {
  ...
  const roles = new UserCollectionRoles()
